...
def guide_report_markdown(
    *,
    ...
    lines = [
        ...
        "## 摘要与导览质量",
        "",
        f"- F-score：`{float(metrics.get('f_score', 0.0)):.3f}`",
        f"- 事件/景点覆盖率：`{float(metrics.get('event_coverage', 0.0)):.3f}`",
        f"- 重复率：`{float(metrics.get('repeat_rate', 0.0)):.3f}`",
        f"- 舒适度分数：`{float(metrics.get('guide_comfort_score', 0.0)):.3f}`",
